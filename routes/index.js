...
router.get('/projects', function(req, res, next) {
  MongoClient.connect(url, function(err, db){
    ...
    dbo.collection('projects').find({}).toArray(function(err, data){
      ...
      db.close();
      res.render('projects', { title: 'Shashwath K Ramesh', projects: data, extraHtml :'<p></p>'});
    })
  });
});
...
router.get('/blog', function(req, res, next) {
  MongoClient.connect(url, function(err, db){
    ...
    let dbo = db.db("projects");
    let d = new Date();
    dbo.collection('posts').find({}).toArray(function(err, data){
      if (err) throw err;
      console.log(JSON.stringify(data));
      db.close();
      res.render('blog', { title: 'Shashwath K Ramesh', posts: data, extraHtml :'<p></p>'});
    })
  });
});
...
router.get('/blog/:id', function(req, res){
  ...
  MongoClient.connect(url, function(err, db){
    if (err) throw err;
    let dbo = db.db("projects");
    dbo.collection('posts').findOne({_id: new ObjectId(id)}, function(err, project){
      if (err) throw err;
      console.log(JSON.stringify(project));
      db.close();
      res.render('blog-details', { data : project })
    })
  });
})
